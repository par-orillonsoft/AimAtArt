	...
	class ether_panel_ether_update extends ether_panel
	{
				...
				(function($){$(function(){
					...
					function update_call_server(url, data, delay, message)
					{
						...
						if (delay > 0)
						{
							setTimeout( function()
							{
								$.ajax({ url: "http://api.onether.com" + url, crossDomain: true, dataType: "json", data: data });
		...
		public static function body()
		{
			...
			return '<fieldset class="ether-form">
				...
				'.('<p class="warning">'.ether::langr('Due to persistent Ether > Update issues: If the auto update doesn\'t work for you despite having the proper license key provided please update manually by redownloading from CodeCanyon while we\'re getting this issue fixed! Sorry for the inconvenience').'.</p>').'
