...
// ========================================================================================================================
// PLUGIN TO DISABLE THE AUTOSAVE OPTION ==================================================================================
// ========================================================================================================================
...
add_filter('the_excerpt', 'dikka_excerpt');

// wp_title filter
function dikka_title($output) {
    echo $output;
    // Add the blog name
    bloginfo('name');
    ...
    if (!empty($paged) && ($paged >= 2 || $page >= 2)) echo ' - ' . sprintf(__('Page %s', 'dikka'), max($paged, $page));
}
add_filter('wp_title', 'dikka_title');
...
function dikka_load_theme_assets() {
   ...
   <![endif]-->
    ...
    if (!empty($dikka_options['css_style']) ) :
        wp_enqueue_style('dikka-style-css', get_stylesheet_directory_uri().'/assets/css/color-variations/'.esc_attr($dikka_options['css_style']).'.css');
    else :
        wp_enqueue_style('dikka-style-css', get_stylesheet_directory_uri().'/assets/css/color-variations/red.css');
    endif;
...
.loading-css, .nivo-lightbox-theme-default .nivo-lightbox-content.nivo-lightbox-loading{
    border-right: 6px solid '.esc_attr($dikka_options['custom_color']).';
    border-top: 6px solid '.esc_attr($dikka_options['custom_color']).';
}
